...
def calculate_external_heat_loads(t, T_TS_ext, T_BS_ext, T_E):
    if not ENABLE_EXTERNAL_RADIATION: return {'Q_ext_top': 0.0, 'Q_ext_bottom': 0.0}
    data_period_seconds = solar_time_data[-1]; current_solar_flux = 0
    ...
    Q_rad_to_sky_TS = -sigma * config.A_TS * config.emis_shell_ext * (T_TS_ext**4)
    total_Q_top = Q_solar_in_TS + Q_rad_to_sky_TS
    albedo_flux = current_solar_flux * FLUX_ALBEDO_FRACTION
    Q_albedo_in_BS = albedo_flux * config.A_BS * config.alpha_solar_shell
    Q_planetary_in_BS = FLUX_PLANETARY * config.A_BS * config.emis_shell_ext
    Q_rad_to_earth_BS = -sigma * config.A_BS * config.emis_shell_ext * (T_BS_ext**4 - T_E**4)
    total_Q_bottom = Q_albedo_in_BS + Q_planetary_in_BS + Q_rad_to_earth_BS
