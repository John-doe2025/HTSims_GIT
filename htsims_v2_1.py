# HTSims_v7_Rerouted_Mount.py

"""
================================================================================
Thermal Analysis of a Nacelle (v7 - Rerouted Mount Path)
================================================================================
Purpose:
This version implements a major design change for heat mitigation. The ESC Mount
is now connected directly to the nacelle shell, creating a dedicated heat path
to the environment. The mount is no longer connected to the battery.
The mount material has been changed to Aluminum for effective heat conduction.
"""
...
# Mount Properties (NOW ALUMINUM 6061)
m_mount = 0.023
C_mount = 896   # Specific Heat of Aluminum (J/kg·K)
A_mount_conv = 0.0067
LC_mount = 0.0087
...
# Geometry and Battery-Battery Conduction
...
# --- MODIFIED: Conduction Paths Rerouted ---
k_mount = 180.0 # Thermal conductivity of Aluminum 6061

# Path 1: From ESC to Mount (Unchanged)
A_contact_ESC_Mount = 0.0012; L_path_ESC_Mount = 0.005
C_cond_ESC_to_Mount = k_mount * A_contact_ESC_Mount / L_path_ESC_Mount

# REMOVED: Path from Mount to Battery Front
# C_cond_Mount_to_Bf = ...

# NEW: Path from Mount to Top Shell Internal
A_contact_Mount_TS = 0.0010; L_path_Mount_TS = 0.003
C_cond_Mount_to_TS = k_mount * A_contact_Mount_TS / L_path_Mount_TS

# Skin Conduction (internal to external shell surface)
k_cfrp = 3; A_cfrp = 0.54168; t_cfrp = 0.0005; C_cond_cfrp = k_cfrp * A_cfrp / t_cfrp
...
C_Mount_rad     = rad_coeff(0.8, 0.2, A_mount_conv, A_TS, 1) # Emissivity for anodized aluminum
...
def natural_convection_h(p_film, T_surface, T_fluid, L_char, is_vertical):
    ...
    Gr = (g * beta * abs(T_surface - T_fluid) * L_char**3) / (nu_val**2 + 1e-12) # Added safety term
    Ra = Gr * Pr; Nu = 1.0
...
def f(t, x):
    ...
    Q_rad_Bm = (C_Bf_TS_int_rad * (T4s['TS_int'] - T4s['Bm']) + C_Bf_TS_int_rad * (T4s['BS_int'] - T4s['Bm'])); Q_rad_Br = (C_Bf_TS_int_rad * (T4s['TS_int'] - T4s['Br']) + C_Bf_TS_int_rad * (T4s['BS_int'] - T4s['Br']))
    ...
    Q_cond_ESC_to_Mount = C_cond_ESC_to_Mount * (T_ESC - T_mount) # Using Source-Sink convention
    ...
    # NEW: Conduction from Mount to Top Shell
    Q_cond_Mount_to_TS = C_cond_Mount_to_TS * (T_mount - T_TS_int)
    ...
    # MODIFIED: Battery Front no longer has conducted heat from the mount
    dT_Bf_dt     = (Q_B_front + Q_cond_Bf + Q_conv_Bf + Q_rad_Bf) / (m_Bf * C_B)
    ...
    # MODIFIED: Mount now loses conducted heat to the Top Shell
    dT_mount_dt  = (Q_cond_ESC_to_Mount - Q_cond_Mount_to_TS + Q_conv_Mount + Q_rad_Mount) / (m_mount * C_mount)
    # MODIFIED: Top Shell now gains conducted heat from the mount
    dT_TS_int_dt = (-Q_cond_TS_ext + Q_conv_TS_in + Q_rad_TS + Q_cond_Mount_to_TS) / (m_TS * C_TS)
...
plt.title('Transient Temperature Evolution (v7 - Rerouted Mount)', fontsize=16)
